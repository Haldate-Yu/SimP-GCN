...
def data_loader(dataset, data_path=datadir, normalization="AugNormAdj", porting_to_torch=True, task_type="full",
                **kwargs):
    if dataset not in ['cora', 'citeseer', 'pubmed', 'reddit']:
        # if True:
        from geom_data_utils import load_geom_datasets
        return load_geom_datasets(dataset, kwargs['seed'])
...
def split_by_degree(adj, thresholds=[1, 10]):
    ...
    print(
        f'high degree range from {degrees[0][1]} - {degrees[len(high) - 1][1]}, percentage of number of nodes: {len(high_) / len(idx)}')
    print(
        f'mid degree range from {degrees[len(high)][1]} - {degrees[len(high) + len(mid) - 1][1]}, percentage of number of nodes: {len(mid_) / len(idx)}')
    print(
        f'low degree range from {degrees[len(high) + len(mid)][1]} - {degrees[-1][1]}, percentage of number of nodes: {len(low_) / len(idx)}')
...
def load_nell(dataset="nell.0.001", normalization="AugNormAdj", porting_to_torch=True, data_path=datadir,
              task_type="full"):
    names = ['x', 'y', 'tx', 'ty', 'allx', 'ally', 'graph']
    objects = []
    for i in range(len(names)):
