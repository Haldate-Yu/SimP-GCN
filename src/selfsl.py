...
class ECTDAttrSim(Base):
    ...
    def regression_loss(self, embeddings):
        if self.pseudo_labels is None:
            agent = ECTDSim(self.adj, self.features, args=self.args)
            self.pseudo_labels = agent.get_label().to(self.device)
            node_pairs = agent.node_pairs
            self.node_pairs = node_pairs
    ...
    def classification_loss(self, embeddings):
        if self.pseudo_labels is None:
            agent = ECTDSim(self.adj, self.features, self.labels, self.args)
            pseudo_labels = agent.get_class()
            self.pseudo_labels = torch.LongTensor(pseudo_labels).to(self.device)
            self.node_pairs = agent.node_pairs
...
class MFPTAttrSim(Base):
    ...
    def regression_loss(self, embeddings):
        if self.pseudo_labels is None:
            agent = MFPTSim(self.adj, self.features, args=self.args)
            self.pseudo_labels = agent.get_label().to(self.device)
            node_pairs = agent.node_pairs
            self.node_pairs = node_pairs
    ...
    def classification_loss(self, embeddings):
        if self.pseudo_labels is None:
            agent = MFPTSim(self.adj, self.features, self.labels, self.args)
            pseudo_labels = agent.get_class()
            self.pseudo_labels = torch.LongTensor(pseudo_labels).to(self.device)
            self.node_pairs = agent.node_pairs
...
class MFPTNormAttrSim(Base):
    ...
    def regression_loss(self, embeddings):
        if self.pseudo_labels is None:
            agent = MFPTNormAttrSim(self.adj, self.features, args=self.args)
            self.pseudo_labels = agent.get_label().to(self.device)
            node_pairs = agent.node_pairs
            self.node_pairs = node_pairs
    ...
    def classification_loss(self, embeddings):
        if self.pseudo_labels is None:
            agent = MFPTNormAttrSim(self.adj, self.features, self.labels, self.args)
            pseudo_labels = agent.get_class()
            self.pseudo_labels = torch.LongTensor(pseudo_labels).to(self.device)
            self.node_pairs = agent.node_pairs
...
class ECTDKNNGraph(Base):
    ...
    def __init__(self, adj, features, nhid, device, idx_train, args):
        ...
        if not os.path.exists(f'saved/{args.dataset}_ectd_sims_{k}.npz'):
            features = np.copy(features)
            features[features != 0] = 1
            # !! knn here !!
            # sims = cosine_similarity(features)
            sims = ectd_similarity(self.adj)
            ...
            # return adj-knn
            self.A_feat = sp.csr_matrix(sims)
            sp.save_npz(f'saved/{args.dataset}_ectd_sims_{k}.npz', self.A_feat)
        else:
            print(f'loading saved/{args.dataset}_ectd_sims_{k}.npz')
            # return adj-knn
            self.A_feat = sp.load_npz(f'saved/{args.dataset}_ectd_sims_{k}.npz')
...
class MFPTKNNGraph(Base):
    ...
    def __init__(self, adj, features, nhid, device, idx_train, args):
        ...
        if not os.path.exists(f'saved/{args.dataset}_mfpt_sims_{k}.npz'):
            ...
            # return adj-knn
            self.A_feat = sp.csr_matrix(sims)
            sp.save_npz(f'saved/{args.dataset}_mfpt_sims_{k}.npz', self.A_feat)
        else:
            print(f'loading saved/{args.dataset}_mfpt_sims_{k}.npz')
            # return adj-knn
            self.A_feat = sp.load_npz(f'saved/{args.dataset}_mfpt_sims_{k}.npz')
...
class NormMFPTKNNGraph(Base):
    ...
    def __init__(self, adj, features, nhid, device, idx_train, args):
        ...
        if not os.path.exists(f'saved/{args.dataset}_mfpt_sims_{k}.npz'):
            ...
            # !! knn here !!
            # sims = cosine_similarity(features)
            sims = norm_mfpt_similarity(self.adj, features)
